...
import com.fahimfarhan.simplevideoeditor.mp4composer.mp4compose.SampleType;
...
class VideoComposer {
    ...
    private int drainExtractor() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            resultByteBuffer = decoder.getInputBuffer(result);
        }else{
            resultByteBuffer = decoder.getOutputBuffers()[result];
        }
        int sampleSizeCompat = mediaExtractor.readSampleData(resultByteBuffer, 0);
    ...
    private int drainEncoder() {
        ...
        ByteBuffer resultByteBuffer = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            resultByteBuffer = encoder.getOutputBuffer(result);
        }else{
            resultByteBuffer = encoder.getOutputBuffers()[result];
        }
        muxRender.writeSampleData(SampleType.VIDEO, resultByteBuffer, bufferInfo);
        writtenPresentationTimeUs = bufferInfo.presentationTimeUs;
        encoder.releaseOutputBuffer(result, false);
        return DRAIN_STATE_CONSUMED;
    }
